...
class RNAToView(object):
    def __init__(self,):
        self.mainscript = mainscript
        self.draw = self.mainscript+'/SVhawkeye.r'
    ...
    def GetOpt(self,genotyping=0):
        ...
        parser.add_option('-g', '--genome', action="store",dest="genome",default="", help='set reference genome; [default:'']')
        parser.add_option('-b', '--bed', action="store", dest="bed",metavar='FILE', help='set the input bed file; [required=True]')
        ...
        parser.add_option('-q', '--quanlty', action="store", dest="quanlty",metavar='num',default=0, help='set reads mapping quanlty for filter; [default: 20 (means Q20)]')
        parser.add_option('-I', '--identity', action="store", dest="identity",metavar=float,default=0, help='set min identity of mapping reads for filter; [default: 0.6]')
        parser.add_option('-d', '--extend', action="store",dest="extend",metavar=int, default=100, help='set region extend length; [default:100bp]')
        ...
        parser.add_option('-p', '--genepred', action="store", dest="genepred",metavar='str',default='None', 
                help='set gene file(gtfToGenePred -genePredExt -ignoreGroupsWithoutExons hg19.ncbi.gtf hg19.genePred ); [default:genome/genome.genePred.gz]')
        (options, args) = parser.parse_args()
        if not options.bams or not options.bed:
            parser.print_help();
            sys.exit(0)
        self.genome = options.genome
        if options.genepred==None:
            genepred = SCRIPTDIR+'/database/{}/{}.genePred.gz'.format(self.genome,self.genome)
            self.genepred = genepred
            if not os.path.exists(genepred):
                print('not found genePred file ;Please provide the genePred file:')
                print('You can run gtfToGenePred -genePredExt -ignoreGroupsWithoutExons genome.ncbi.gtf genome.genePred')
                parser.print_help();
                sys.exit(0)
        else:
            self.genepred = options.genepred
        self.bams = options.bams.split(',')
        for i in self.bams:isfile(i)
        self.reffa = options.reffa
        self.bed = os.path.realpath(options.bed)
        self.outdir = os.path.realpath(options.outdir)
        self.thread = max(int(options.thread),1)
        self.minMapq = int(options.quanlty)
        self.SV_min_length = 50 #int(options.sv_min_length)
    ...
    def bed2pysamout(self,bedlist): # single row
        ...
        if TRA:
            ...
            work = ("%s --input %s --main %s --samples %s --outpng %s --genome %s --main2 %s --input2 %s --splitreads %s --RNA TRUE --genepred %s"
                    %(self.draw, inputpysamout, main1, samples, outpng,self.genome, main2, inputpysamout2, splitrfile,self.genepred) )
        else:
            work = ("%s --input %s --main %s --samples %s --outpng %s --genome %s --RNA TRUE --genepred %s"
                    %(self.draw, inputpysamout, main1, samples, outpng, self.genome,self.genepred) )
